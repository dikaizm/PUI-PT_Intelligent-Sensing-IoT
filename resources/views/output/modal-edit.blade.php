...
<div class="modal fade" {{-- id="modalEditOutput{{ $user->id }}" --}} tabindex="-1" aria-labelledby="ModalFourLabel" aria-hidden="true"
  style="">
          ...
          <form action="{{ route('profile.update') }}" method="POST">
            ...
            <div class="d-flex align-items-start">
              <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                <button class="nav-link active" id="v-pills-judul-tab" data-bs-toggle="pill"
                  data-bs-target="#v-pills-judul" type="button" role="tab" aria-controls="v-pills-judul"
                  aria-selected="true">
                  {{ __('Nama') }}
                </button>
                <button class="nav-link" id="v-pills-publikasi-tab" data-bs-toggle="pill"
                  data-bs-target="#v-pills-publikasi" type="button" role="tab" aria-controls="v-pills-publikasi"
                  aria-selected="false">
                  {{ __('Publikasi') }}
                </button>
              ...
              <div class="col-lg-10 tab-content" id="v-pills-tabContent">
                <div class="tab-pane fade show active" id="v-pills-judul" role="tabpanel"
                  aria-labelledby="v-pills-judul-tab">
                  <div class="input-style-1">
                    <label for="name">{{ __('Nama Penelitian') }}</label>
                    <input @error('name') class="form-control is-invalid" @enderror type="text" name="name"
                      id="name" placeholder="{{ __('Nama Penelitian') }}"
                ...
                <div class="tab-pane fade" id="v-pills-publikasi" role="tabpanel"
                  ...
                  <div class="input-style-1">
                    ...
                    <input @error('name') class="form-control is-invalid" @enderror type="text" name="name"
                      id="name" placeholder="{{ __('Skema Penelitian') }}"
                      value="{{ old('name', auth()->user()->name) }}">
                    @error('name')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                  ...
                  <div class="input-style-1">
                    ...
                    <div id="inputContainer">
                      <div class="input-style-1">
                        <input type="text" name="name" {{-- id="nameEdit{{ $user->id }}" --}} placeholder="{{ __('Name') }}"
                          {{-- value="{{ old('name', $user->name) }}" --}} required class="form-control @error('name') is-invalid @enderror">
                        @error('name')
                          <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                          </span>
                  ...
                  <div class="input-style-1">
                    ...
                    <select class="form-control @error('status_penelitian') is-invalid @enderror"
                      name="status_penelitian" id="statusPenelitian{{ $item->id }}"
                      style="max-width: 100%; margin: 0 auto;">
                      @foreach ($status_penelitian as $status)
                        <option value="{{ $status->id }}" @if (old('status_penelitian', $item->statusPenelitian->id) == $status->id) selected @endif>
                  ...
                  <div class="input-style-1">
                    <label for="nameEdit{{ $user->id }}">{{ __('Tanggal Publish') }} <span
                  ...
                  <div class="input-style-1">
                    <label for="name">{{ __('Link Jurnal') }} <span
                ...
                <div class="tab-pane fade" id="v-pills-hki" role="tabpanel" aria-labelledby="v-pills-hki-tab">
                  ...
                  <div class="input-style-1">
                    <label for="nameEdit{{ $user->id }}">{{ __('Tanggal Granted') }} <span
                  ...
                  <div class="input-style-1">
                    <label for="name">{{ __('Link HKI') }} <span
                        style="color:gray;">{{ __('*Apabila sudah publish') }}</span> </label>
                    <input @error('name') class="form-control is-invalid" @enderror type="text" name="name"
                      id="name" placeholder="{{ __('Skema Penelitian') }}"
                      value="{{ old('name', auth()->user()->name) }}">
                ...
                <div class="tab-pane fade" id="v-pills-foto" role="tabpanel" aria-labelledby="v-pills-foto-tab">
                  <div class="tab-pane fade show active" id="v-pills-judul" role="tabpanel"
                    ...
                    <div class="input-style-1">
                      ...
                      <div class="input-style-1">
                        <input type="file" accept=".pdf" @error('name') class="form-control is-invalid" @enderror
