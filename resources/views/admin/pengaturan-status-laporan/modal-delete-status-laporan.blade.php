@foreach ($status_laporan as $item)
    <div class="modal fade" id="modalDeleteStatusLaporan{{ $item->id }}" tabindex="-1" aria-labelledby="ModalFourLabel"
        aria-hidden="true">
            ...
            <div class="modal-content card-style">
                ...
                <div class="modal-body px-0">
                    <div class="content mb-30">
                        <p class="text-sm">
                            {{ __('Are you sure you want to delete "' . $item->statusLaporanKey->name . ' ' . $item->name . '"?') }}
                        </p>
                    </div>
                    <div class="action d-flex flex-wrap justify-content-end">
                        <form action="{{ route('status-laporan.destroy', ['id' => $item->id]) }}" method="POST">
                            @method('DELETE')
                            @csrf
