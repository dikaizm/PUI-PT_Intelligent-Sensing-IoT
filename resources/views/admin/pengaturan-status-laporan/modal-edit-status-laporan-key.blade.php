@foreach ($status_laporan_key as $item)
    <div class="modal fade" id="modalEditStatusLaporanKey{{ $item->id }}" tabindex="-1"
            ...
            <div class="modal-content card-style">
                ...
                <div class="modal-body px-0">
                    <div class="content mb-30">
                        <form action="{{ route('status-laporan-key.update', ['id' => $item->id]) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="input-style-1">
                                        <label
                                            for="nameEditStatusLaporanKey{{ $item->id }}">{{ __('Name') }}</label>
                                        <input type="text" @error('name') class="form-control is-invalid" @enderror
                                            name="name" id="nameEditStatusLaporanKey{{ $item->id }}"
